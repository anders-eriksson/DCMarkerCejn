...
namespace DCAdmin
{
    public class DB : IDisposable
    {
        ...
        public bool ExistsLaserData(string machineCode, string article, string kant)
        {
            bool result = false;

            var found = _context.LaserData.FirstOrDefault(p => p.F1 == article && p.Kant == kant && p.MachineCode == machineCode);
